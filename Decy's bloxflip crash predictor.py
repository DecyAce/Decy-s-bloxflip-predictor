...
def get_crash_numbers():
    ...
    if len(crash_numbers) == 7 and below_2 == 7:
        result_label.config(
            text="Possible big win coming up (only join if you can afford to lose or are placing low bets)")
    else:
...
def show_credits():
    ...
    label_credits = tk.Label(credits_window, text="Developed By Dexter", font=("Arial", 14))
    label_credits.pack()
...
def show_how_it_works():
    ...
    explanation_text = (
        ...
        "it's a good idea to join the next game or not."
    )
